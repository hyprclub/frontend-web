...
import { AVAILNFT_MODAL_CLOSE_SUCCESS, AVAILNFT_MODAL_OPEN_SUCCESS } from '../../../redux/constants/availnftModal';
...
const Perks = ({ item, Bought }: Props) => {
    const { clicked_a } = useSelector((state: RootStore) => state.availnftModalOpen)
    ...
    return (
        <>
            ...
            {Bought && <GradBorder className={styles.avail_perk_button} text="Avail Perk" onClick={AvailPerks} />}
            <div className={clsx(styles.modalDiv, clicked_a ? styles.show : styles.hide)} onClick={closeModal}>
                <Avail_modals />
            </div>
        </>
    );
